...
def update_order_status(request, order_id):
    ...
    return redirect("vendor:order_detail", order.order_id, context)
...
def reviews(request):
    ...
    if rating:
        reviews = reviews.filter(rating=rating)  # Apply filter to the reviews_list

    if date:
        reviews = reviews.order_by(date)  # Ensure this refers to a valid model field
